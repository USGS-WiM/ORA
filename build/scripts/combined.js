(function ($) {
  $.fn.confirmModal = function (opts) {
    ...
    return this.each(function (index) {
      ...
      var modalId = "confirmModal" + Math.floor(Math.random() * 1e9);
      ...
      $('button[data-dismiss="ok"]', confirmModal).on(
        "click",
        function (event) {
...
require([
  ...
  "dojo/domReady!",
], function (Extent, WMSLayerInfo, FeatureLayer) {
  allLayers = [
    //{
...
require([
  ...
  esriConfig.defaults.geometryService = new GeometryService(
    "https://gis.wim.usgs.gov/arcgis/rest/services/Utilities/Geometry/GeometryServer"
  );
  ...
  var utmCoords = $(
    '<tr class="esriMeasurementTableRow" id="utmCoords"><td><span>UTM17</span></td><td class="esriMeasurementTableCell"> <span id="utmX" dir="ltr">UTM X</span></td> <td class="esriMeasurementTableCell"> <span id="utmY" dir="ltr">UTM Y</span></td></tr>'
  );
  ...
  function showShareModal() {
    ...
    var shareQueryString =
      "?xmax=" +
      currentMapExtent.xmax.toString() +
      "&xmin=" +
      currentMapExtent.xmin.toString() +
      "&ymax=" +
      currentMapExtent.ymax.toString() +
      "&ymin=" +
      currentMapExtent.ymin.toString();
    var encodedShareQueryString =
      "%3Fxmax=" +
      currentMapExtent.xmax.toString() +
      "%26xmin=" +
      currentMapExtent.xmin.toString() +
    ...
    $("#showFullLinkButton").click(function () {
      $("#fullShareURL").html(
        '<span id="fullLinkLabel" class="label label-default"><span class="glyphicon glyphicon-link"></span> Full link</span><br><textarea style="margin-bottom: 10px; cursor: text" class="form-control"  rows="3" readonly>' +
          shareURL +
          "</textarea>"
      );
      gtag("event", "click", {
        event_category: "Share Modal",
        event_label: "Display Full Link",
      });
    });
    ...
    $("#showShortLinkButton").click(function () {
      $.ajax({
        ...
        type: "GET",
        url:
          "https://api-ssl.bitly.com/v3/shorten?access_token=e1a16076cc8470c65419920156e0ae2c4f77850f&longUrl=" +
          encodedShareURL,
        headers: { Accept: "*/*" },
        success: function (data) {
          var bitlyURL = data.data.url;
          $("#bitlyURL").html(
            '<span class="label label-default"><span class="glyphicon glyphicon-link"></span> Bitly link</span><code>' +
              bitlyURL +
              "</code>"
          );
          gtag("event", "click", {
            event_category: "Share Modal",
            event_label: "Generate Bitly Link",
          });
        },
        error: function (error) {
          $("#bitlyURL").html(
            '<i class="fa fa-exclamation-triangle"></i> An error occurred retrieving shortened Bitly URL'
          );
        },
      });
    });
  ...
  var shadedReliefBasemap = new ArcGISTiledMapServiceLayer(
    "https://server.arcgisonline.com/arcgis/rest/services/World_Shaded_Relief/MapServer",
    { visible: false }
  );
  var referenceMapBasemap = new ArcGISTiledMapServiceLayer(
        ...
        if (noExtentCheck == -1) {
          ...
          require(["esri/geometry/Point"], function (Point) {
            ...
            gtag("event", "click", {
              ...
              event_label: "Location Search Performed",
            });
          });
        }
      });
  ...
  function printMap() {
    ...
    if (userTitle === "") {
      template.layoutOptions = {
        ...
        copyrightText:
          "This page was produced by the LORA web application at glcwra.wim.usgs.gov/LORA",
    ...
    var printMap = new PrintTask(
      "https://gis.wim.usgs.gov/arcgis/rest/services/Utilities/PrintingTools/GPServer/Export%20Web%20Map%20Task"
    );
    ...
    function printError(event) {
      alert(
        "Sorry, unable to print this far zoomed-out, try Zooming in and then printing."
      );
      $("#printExecuteButton").button("reset");
    }
  }
  function saveUserBookmark() {
    ...
    if (userBookmarkTitle.length > 0) {
      ...
      var userBookmarkButton = $(
        '<tr id="' +
          userBookmarkID +
          '"><td  class="bookmarkTitle td-bm">' +
          userBookmarkTitle +
          '</td><td class="text-right text-nowrap"> <button id="' +
          bmDeleteID +
          '" class="btn btn-xs btn-warning bookmarkDelete" data-toggle="tooltip" data-placement="top" > <span class="glyphicon glyphicon-remove"></span> </button> </td> </tr>'
      );
      $("#bookmarkList").append(userBookmarkButton);

      gtag("event", "click", {
        event_category: "Sidebar",
        event_label: "User saved bookmark",
      });
      gtag("event", "input", {
        event_category: "Sidebar",
        event_label: "User saved bookmark titled: " + userBookmarkTitle,
      });
  ...
  function mapReady() {
    ...
    if (urlSiteObject.query) {
      var urlExtent = new Extent(
        parseFloat(urlSiteObject.query.xmin),
        parseFloat(urlSiteObject.query.ymin),
        parseFloat(urlSiteObject.query.xmax),
        parseFloat(urlSiteObject.query.ymax),
  ...
  $(document).ready(function () {
      ...
      if (bm.userCreated === false) {
        var bookmarkButton = $(
          '<tr id="' +
            ...
            '"><td class="bookmarkTitle td-bm">' +
            bm.name +
            '</td><td class="text-right text-nowrap"></td> </tr>'
        );
        ...
        var userBookmarkButton = $(
          '<tr id="' +
            ...
            '</td><td class="text-right text-nowrap"> <button id="' +
            bmDeleteID +
            '" class="btn btn-xs btn-warning bookmarkDelete" data-toggle="tooltip" data-placement="top" title="Delete bookmark"> <span class="glyphicon glyphicon-remove"></span> </button> </td> </tr>'
        );
        $("#bookmarkList").append(userBookmarkButton);
    ...
    $("body").on("click", ".td-bm", function () {
      ...
      lora.bookmarks.forEach(function (bookmark) {
        if (bookmark.id == bookmarkID) {
          var bookmarkExtent = new Extent(
            bookmark.xmin,
            bookmark.ymin,
            bookmark.xmax,
            bookmark.ymax,
            new SpatialReference(bookmark.spatialReference)
          );
    ...
    $("#removeBookmarksButton").confirmModal({
      ...
      confirmMessage:
        "This action will remove all user-defined bookmarks from local memory on your computer or device. Would you like to continue?",
  ...
  require([
    ...
    const mapServiceRoot =
      "https://gis.wim.usgs.gov/arcgis/rest/services/GLCWRA/";
    const geomService = new GeometryService(
      "https://gis.wim.usgs.gov/arcgis/rest/services/Utilities/Geometry/GeometryServer"
    );
    ...
    const dikesLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "LORA/MapServer",
      { id: "dikes", visible: false, minScale: 100000 }
    );
    ...
    const degFlowlinesLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "LORA/MapServer",
      { id: "degFlowlines", visible: false, minScale: 100000 }
    );
    ...
    const culvertsLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "LORA/MapServer",
      { id: "culverts", visible: false, minScale: 100000 }
    );
    ...
    zonalStatsGP = new Geoprocessor(
      "https://gis.wim.usgs.gov/arcgis/rest/services/GLCWRA/LORAZonalStats/GPServer/LORAZonalStats"
    );
    ...
    on(customAreaDraw, "DrawEnd", function (customAreaGeometry) {
      ...
      customAreaSymbol = new SimpleFillSymbol(
    ...
    function displayCustomStatsResults(customStatsResults) {
      ...
      zonalStatsTable.html(
        "<tr><th>Mean </th><th>Standard Deviation</th><th>Max</th></tr>"
      );
      zonalStatsTable.append(
        "<tr><td>" +
          results.MEAN.toFixed(4) +
          "</td><td>" +
          results.STD.toFixed(3) +
          "</td><td>" +
          results.MAX +
          "</td></tr>"
      );
    ...
    const normRestorationIndexLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "LORA/MapServer",
      { id: "normalized", visible: true }
    );
    ...
    const studyAreaLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "LORA/MapServer",
      { id: "studyArea", visible: true }
    );
    ...
    const waterMaskLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "LORA/MapServer",
      { id: "waterMask", visible: true, opacity: 0.75 }
    );
    ...
    vegPopup.setContent(
      "<div style='text-align: left'><b>Wetland:</b>  ${name}<br/><b>Wetland class:</b> ${class}<br/><b>Veg IBI value:</b> ${VegIBI}<br/>More information available from the Great Lakes Coastal Wetlands Monitoring Program: <a href='http://greatlakeswetlands.org' target='_blank'>greatlakeswetlands.org</a></div>"
    );

    var vegLayer = new FeatureLayer(
      "https://services5.arcgis.com/ed839pyDNWVlk9KK/ArcGIS/rest/services/CWMP_Vegetation_IBI/FeatureServer/0",
      {
        id: "veg",
        layerID: "veg",
        visible: false,
    ...
    legendLayers.push({
      layer: vegLayer,
      title: "Wetland Biological Integrity (IBI score)",
    });
    ...
    ///parameters group
    const landuseLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "LORA/MapServer",
      { id: "landuse", visible: false }
    );
    ...
    const imperviousSurfacesLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "LORA/MapServer",
      { id: "imperviousSurfaces", visible: false }
    );
    ...
    const conservedLandsLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "LORA/MapServer",
      { id: "conservedLands", visible: false }
    );
    ...
    const flowlineLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "LORA/MapServer",
      { id: "flowline", visible: false }
    );
    ...
    const wetsoilsLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "LORA/MapServer",
      { id: "wetsoils", visible: false }
    );
    ...
    const hydroperiodLayer = new ArcGISDynamicMapServiceLayer(
      mapServiceRoot + "LORA/MapServer",
      { id: "hydroperiod", visible: false }
    );
    ...
    $("#hydroConditionGroup, #parametersGroup, #4scaleGroup").on(
      "hide.bs.collapse",
      function () {
  ...
  $(document).ready(function () {
    ...
    $("#metadataTab").click(function () {
      gtag("event", "click", {
        event_category: "About Modal",
        event_label: "Layer Information",
      });
    });
    ...
    $("#updatesTab").click(function () {
      gtag("event", "click", {
        event_category: "About Modal",
        event_label: "Get Updated",
      });
    });
    $("#mailingList").click(function () {
      gtag("event", "click", {
        event_category: "About Modal",
        event_label: "Link to Mailing List",
      });
    });
    $("#wim_logo").click(function () {
      gtag("event", "click", {
        event_category: "WIM Ref",
        event_label: "About Footer WIM Ref",
      });
    });
    $("#wimPower").click(function () {
      gtag("event", "click", {
        event_category: "WIM Ref",
        event_label: "Powered by WIM Ref",
      });
    });
    $("#locateButton").click(function () {
      gtag("event", "click", {
        event_category: "Map Button",
        event_label: "Find My Location",
      });
    });
    $("#homeButton").click(function () {
      gtag("event", "click", {
        event_category: "Map Button",
        event_label: "Default Extent",
      });
    });
    $("#normalized").click(function () {
      gtag("event", "click", {
        event_category: "Layer - Restoration Model",
        event_label: "LORA Composite Index",
      });
    });
    $("#waterMask").click(function () {
      gtag("event", "click", {
        event_category: "Layer - Parameters",
        event_label: "water Mask (P0)",
      });
    });
    $("#hydroperiod").click(function () {
      gtag("event", "click", {
        event_category: "Layer - Parameters",
        event_label: "Hydroperiod (P1)",
      });
    });
    $("#wetsoils").click(function () {
      gtag("event", "click", {
        event_category: "Layer - Parameters",
        event_label: "Wetsoils (P2)",
      });
    });
    $("#flowline").click(function () {
      gtag("event", "click", {
        event_category: "Layer - Parameters",
        event_label: "Flowline (P3)",
      });
    });
    $("#conservedLands").click(function () {
      gtag("event", "click", {
        event_category: "Layer - Parameters",
        event_label: "Conserved Lands (P4)",
      });
    });
    $("#imperviousSurfaces").click(function () {
      gtag("event", "click", {
        event_category: "Layer - Parameters",
        event_label: "Impervious Surfaces (P5)",
      });
    });
    $("#landuse").click(function () {
      gtag("event", "click", {
        event_category: "Layer - Parameters",
        event_label: "Landuse (P6)",
      });
    });
    $("#studyArea").click(function () {
      gtag("event", "click", {
        event_category: "Layer - Reference",
        event_label: "Study Area Boundary",
      });
    });
    $("#culverts").click(function () {
      gtag("event", "click", {
        event_category: "Layer - Hydro Conditioning",
        event_label: "Culverts",
      });
    });
    $("#degFlowlines").click(function () {
      gtag("event", "click", {
        event_category: "Layer - Hydro Conditioning",
        event_label: "Degree Flowlines",
      });
    });
